...
const Login = () => {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-6 font-mono relative overflow-hidden transition-colors duration-500 ${isLight ? "bg-stone-100" : "bg-[#0a0908]"}`}>
      {/* Efecto CRT (Oculto en modo claro para limpieza visual) */}
      {!isLight && (
      ...
      >
        ...
        <div className={`w-24 h-24 border-2 rounded-full flex items-center justify-center mb-8 overflow-hidden transition-colors ${isLight ? "bg-stone-50 border-stone-100" : "bg-amber-500/5 border-amber-500/20"}`}>
          <img src={BrachioSkull} alt="Logo" className={`w-[80%] h-[80%] object-contain transition-all ${isLight ? "opacity-90 grayscale-[0.2]" : "opacity-80 brightness-110 sepia-[0.5]"}`} />
        </div>

        <div className="text-center mb-10">
          <h1 className={`text-3xl font-black italic tracking-tighter uppercase leading-none transition-colors ${isLight ? "text-stone-900" : "text-[#fef3c7]"}`}>INICIO DE SESIÓN</h1>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6 w-full">
          ...
          <button type="submit" className="w-full bg-amber-600/10 hover:bg-amber-600/25 border-2 border-amber-600/60 text-amber-600 py-5 rounded-3xl font-black italic uppercase tracking-[0.2em] text-lg transition-all mt-4 group flex items-center justify-center gap-3.5">
