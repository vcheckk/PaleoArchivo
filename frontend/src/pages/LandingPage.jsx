...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#1d1914] px-4 pt-4 pb-20 relative text-white overflow-x-hidden">
      ...
      <div 
        className={`fixed left-0 right-0 z-[100] mx-auto w-full max-w-md px-4 transition-all duration-700 ease-out pointer-events-none ${
          showLogoutMsg ? "top-32 opacity-100 scale-100" : "top-0 opacity-0 scale-95"
        }`}
      >
        <div className="bg-[#1a1614]/95 border border-amber-600/40 backdrop-blur-md rounded-xl shadow-[0_20px_60px_-10px_rgba(0,0,0,0.8)] overflow-hidden pointer-events-auto">
          <div className="px-5 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-bold text-[11px] uppercase tracking-wider leading-tight">Sesión Cerrada</p>
                <p className="text-stone-500 font-mono text-[9px] uppercase tracking-widest leading-none">Acceso restringido activado</p>
              </div>
            </div>
            <button onClick={() => setShowLogoutMsg(false)} className="text-stone-600 hover:text-white transition-colors p-1">
              <X size={16} />
            </button>
          </div>
      ...
      <div className="max-w-6xl mx-auto text-center w-full relative z-10">
        ...
        {/* BUSCADOR - TAMAÑO INCREMENTADO */}
        <div className="max-w-2xl mx-auto relative z-40 mb-10">
          <div className="relative group">
            <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none">
              {/* Icono más grande (size 22) */}
            ...
            <input
              ...
              /* Padding py-5 y text-base para que sea más grande */
              className="w-full bg-[#2a2420]/40 border border-stone-800 py-5 md:py-6 pl-16 pr-14 rounded-2xl text-sm md:text-base font-mono tracking-widest focus:outline-none focus:border-amber-600/50 focus:bg-[#2a2420]/80 transition-all placeholder:text-stone-700 text-white uppercase shadow-2xl"
            />
            {searchTerm && (
              <button onClick={() => setSearchTerm("")} className="absolute inset-y-0 right-6 flex items-center text-stone-500 hover:text-white transition-colors">
                <X size={22} />
              </button>
            )}
          </div>

          {/* DROP-DOWN DE RESULTADOS - TAMAÑO INCREMENTADO */}
          {searchTerm && (
            <div className="absolute w-full mt-3 bg-[#1a1614] border border-stone-800 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-xl max-h-[450px] overflow-y-auto z-50">
              {filteredDinos.length > 0 ? (
                filteredDinos.map((dino) => {
                  const theme = themes[dino.dieta] || { text: "text-white", bg: "bg-white/10", border: "border-white/20" };
                  return (
                    /* Padding p-5 y gap-5 para que respire más */
                    <Link key={dino.id} to={`/animal/${dino.nombre.toLowerCase()}`} className="flex items-center justify-between p-5 hover:bg-amber-600/10 transition-colors border-b border-white/5 last:border-none group/item">
                      <div className="flex items-center gap-5">
                        ...
                        <div className="text-left">
                          ...
                          <p className="font-black uppercase italic text-white group-hover/item:text-amber-500 transition-colors leading-none text-base">{dino.nombre}</p>
          ...
          {Object.keys(themes).map((dieta) => {
            ...
            return (
              <button
                ...
                className={`w-full px-2 py-3 rounded-xl border flex items-center justify-center gap-2 transition-all duration-300 ${theme.bg} ${theme.border} ${theme.text} ${isActive ? "ring-2 ring-amber-500 scale-[1.05] opacity-100 shadow-lg" : "opacity-60 hover:opacity-100 hover:scale-[1.02]"}`}
              >
                <span className="text-2xl filter saturate-150 shrink-0">{dietaEmojis[dieta]}</span>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        {eras.map((era) => (
          <EraCard key={era.id} id={era.id} name={era.name} age={era.age} image={era.image}>
            {era.desc}
          </EraCard>
        ))}
      </div>
