...
const FavoritesPage = () => {
  ...
  if (loading)
    return (
      <div
        className={`min-h-screen flex items-center justify-center font-mono text-sm italic tracking-[0.4em] ${isLight ? "bg-[#f7f3ee] text-stone-400" : "bg-[#0c0b0a] text-amber-600"}`}
      >
        {fav.loading}
      </div>
    );
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 pb-20">
        ...
        {myFavs.length === 0 ? (
          <div
            className={`flex flex-col items-center justify-center py-32 rounded-2xl border ${isLight ? "border-stone-200 bg-white" : "border-[#2a2520] bg-[#131211]"}`}
          >
            <FolderHeart
              size={64}
              className={`mb-6 ${isLight ? "text-stone-300" : "text-stone-700"}`}
            />
            <h3
              className={`text-sm font-black uppercase tracking-widest italic mb-8 text-center px-4 ${isLight ? "text-stone-400" : "text-stone-600"}`}
            >
              {fav.emptyTitle}
            </h3>
            <Link
          ...
          /* ╔══════════════════════════════════════╗
             ...
             ╚══════════════════════════════════════╝ */
          <div
            className={`flex flex-col md:flex-row rounded-2xl overflow-hidden border ${isLight ? "border-stone-200" : "border-[#2a2520]"}`}
          >
            {/* ╔══════════════════════════════════╗
                ║   PANEL IZQUIERDO — Stats y filtros ║
                ╚══════════════════════════════════╝ */}
            <div
              className={`md:w-52 shrink-0 flex flex-col p-6 md:border-r ${isLight ? "bg-[#f0ebe3] border-stone-200" : "bg-[#0f0e0c] border-[#2a2520]"}`}
            >
              {/* Acento decorativo */}
              ...
              {/* Título y contador */}
              <p
                className={`text-[12px] tracking-[0.2em] uppercase mb-2 ${isLight ? "text-stone-400" : "text-stone-600"}`}
              >
              ...
              <p
                className={`text-[12px] tracking-[0.16em] uppercase mt-1 mb-5 ${isLight ? "text-stone-400" : "text-stone-600"}`}
              >
                especímenes
              </p>
              ...
              <div
                className={`flex flex-col gap-3 pb-5 border-b ${isLight ? "border-stone-200" : "border-[#2a2520]"}`}
              >
                {dietFilters.map(([diet, count]) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`text-[13px] tracking-[0.08em] uppercase ${isLight ? "text-stone-500" : "text-stone-500"}`}
                      >
                        {cfg.emoji} {getDietLabel(diet, lang)}
                      </span>
                      <span
                        className={`text-base font-black ${cfg.color.text}`}
                      >
                        {count}
                      </span>
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`text-[11px] tracking-[0.08em] uppercase ${isLight ? "text-stone-500" : "text-stone-500"}`}
                  >
                    🌍 Eras
                  </span>
                  <span
                    className={`text-sm font-black ${isLight ? "text-stone-700" : "text-stone-300"}`}
                  >
                    {stats.eras}
                  </span>
                </div>
              ...
              <div className="mt-5 flex flex-col gap-1.5">
                <p
                  className={`text-[14px] tracking-[0.16em] uppercase mb-3 ${isLight ? "text-stone-400" : "text-stone-600"}`}
                >
                  Filtrar
                </p>
                <button
                  onClick={() => setActiveFilter("all")}
                  className={`text-left px-3 py-2 rounded-lg text-[13px] tracking-[0.1em] uppercase border transition-all font-bold
                    ${
                      activeFilter === "all"
                ...
                {dietFilters.map(([diet, count]) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-left px-3 py-2 rounded-lg text-[12px] tracking-[0.1em] uppercase border transition-all font-bold
                        ${
                          isActive
            ...
            {/* ╔════════════════════════════════╗
                ...
                ╚════════════════════════════════╝ */}
            <div
              className={`flex-1 flex flex-col ${isLight ? "bg-white" : "bg-[#131211]"}`}
            >
              {/* Buscador */}
              <div
                className={`flex items-center gap-3 px-4 border-b ${isLight ? "border-stone-100" : "border-[#2a2520]"}`}
              >
                <Search
                  size={20}
                  className={`shrink-0 ${isLight ? "text-stone-300" : "text-stone-700"}`}
                />
                <input
                  value={search}
                  ...
                  {filtered.length > 0 ? (
                    filtered.map((animal) => {
                      ...
                      return (
                        ...
                        >
                          <Link
                            to={`/animal/${animal.nombre.toLowerCase()}`}
                          ...
                          >
                            {/* Thumbnail */}
                            <div
                              className={`w-14 h-12 shrink-0 overflow-hidden rounded-lg border ${isLight ? "border-stone-100" : "border-[#2a2520]"}`}
                            >
                              <img
                                src={animal.imagen}
                                alt={animal.nombre}
                            ...
                            <div className="flex-1 min-w-0">
                              <p
                                className={`text-xl font-black italic uppercase tracking-tight leading-tight group-hover:text-amber-500 transition-colors truncate ${isLight ? "text-stone-900" : "text-[#f5e6c8]"}`}
                              >
                                {animal.nombre}
                              </p>
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                {animal.periodo && (
                                  <span
                                    className={`text-[10px] tracking-[0.1em] uppercase ${isLight ? "text-stone-400" : "text-stone-600"}`}
                                  >
                                    {animal.periodo}
                                  </span>
                                )}
                                <span
                                  className={`text-[12px] tracking-[0.06em] uppercase px-2 py-0.5 rounded-md ${cfg.color.bg} ${cfg.color.text}`}
                                >
                    ...
                    <div
                      className={`text-center py-20 text-sm uppercase tracking-widest ${isLight ? "text-stone-400" : "text-stone-600"}`}
                    >
                      Sin resultados
                    </div>
                  )}
