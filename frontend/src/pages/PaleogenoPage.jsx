...
const PaleogenoPage = () => {
  ...
  const periodos = [
    { id: 'paleoceno', name: 'PALEOCENO', age: '66 - 56 m.a.', image: 'https://eltamiz.com/elcedazo/wp-content/uploads/2013/11/bosque-del-paleoceno.jpg', desc: ep.paleogeno?.paleoceno?.desc },
    { id: 'eoceno',    name: 'EOCENO',    age: '56 - 34 m.a.', image: 'https://www.lifeder.com/wp-content/uploads/2019/01/Eoceno.jpg', desc: ep.paleogeno?.eoceno?.desc },
    { id: 'oligoceno', name: 'OLIGOCENO', age: '34 - 23 m.a.', image: 'https://i.pinimg.com/736x/97/af/53/97af53e9b6ba8d39d281a4f80e746f0f.jpg', desc: ep.paleogeno?.oligoceno?.desc },
  ];
    ...
    >
      <div className="max-w-[1400px] mx-auto mb-16">
        ...
        <h1 className={`text-5xl md:text-7xl font-black tracking-tighter italic leading-none transition-colors ${isLight ? 'text-stone-900' : 'text-white'}`}>
          {ep.epochsOf} <span className="text-amber-600">PALEÓGENO</span>
        </h1>
        <div className="flex items-center gap-4 mt-4">
          ...
          <p className={`font-mono text-xs uppercase tracking-[0.2em] transition-colors ${isLight ? 'text-stone-500' : 'text-slate-500'}`}>
            // {ep.paleogeno?.subtitle}
          </p>
        </div>
      </div>
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        {periodos.map((p) => (
          <EraCard key={p.id} id={`cenozoico/paleogeno/${p.id}`} name={p.name} age={p.age} image={p.image}>{p.desc}</EraCard>
        ))}
      </div>
    </motion.div>
  );
