...
const Register = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0908] flex items-center justify-center p-6 font-mono relative overflow-hidden">
      ...
      >
        {/* ICONO DE VOLVER (ESQUINA INTERNA) */}
        <button 
          onClick={() => navigate("/")}
        ...
        <div className="text-center mb-10 flex flex-col items-center gap-2.5">
          <h1 className="text-3xl font-black text-[#fef3c7] italic tracking-tighter uppercase leading-none text-center">
        ...
        <form onSubmit={handleRegister} className="space-y-6 w-full">
          ...
          <div className="space-y-2">
            <label className="text-[12px] text-stone-500 tracking-widest ml-5 font-bold uppercase">Correo Electrónico</label>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-black/40 border-2 border-[#3f3833] rounded-3xl py-5 pl-14 pr-5 text-[#fef3c7] text-lg focus:outline-none focus:border-amber-500/60 font-bold uppercase placeholder:text-stone-800" 
                placeholder="CORREO@PALEOMAIL.COM" 
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-[12px] text-stone-500 tracking-widest ml-5 font-bold uppercase">Confirmar Contraseña</label>
            <div className="relative">
              <Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-stone-600" size={20} />
              <input 
                type="password" 
          ...
          <button type="submit" className="w-full bg-amber-600/10 hover:bg-amber-600/25 border-2 border-amber-600/60 text-amber-500 py-5 rounded-3xl font-black italic uppercase tracking-[0.2em] text-lg transition-all mt-4 group flex items-center justify-center gap-3.5 shadow-lg shadow-amber-900/10">
        ...
        <div className="mt-8 pt-6 border-t-2 border-[#3f3833]/70 w-full text-center">
          <p className="text-[14px] text-stone-600 uppercase tracking-widest font-light">
            ¿Ya tiene una cuenta? <Link to="/login" className="text-amber-500/60 font-bold hover:text-amber-500 transition-colors">INICIAR SESIÓN</Link>
          </p>
        </div>
      </motion.div>
    </div>
